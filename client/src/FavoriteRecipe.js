...
const tableIcons = {
  ...
  class FavouriteRecipe extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
      getUID().then(user => {
        fetch(util.format('%s/api/favorite?uid=%s', process.env.REACT_APP_EXPRESS_BACKEND, user.uid), {
          method: "GET",
          headers: {
  ...
  export default withStyles(styles)(FavouriteRecipe);
